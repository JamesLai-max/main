...
class AutoClickApp:
    ...
    def build_ui(self):
        ...
        ttk.Button(frame, text="讀取目前滑鼠位置 (F9)", command=self.capture_mouse_position) \
            .grid(row=3, column=0, columnspan=2, sticky="ew", pady=8)

        ttk.Button(frame, text="開始點擊 (F8)", command=self.start_clicking) \
            .grid(row=4, column=0, sticky="ew", pady=6, padx=(0, 4))

        ttk.Button(frame, text="停止點擊 (F8)", command=self.stop_clicking) \
            .grid(row=4, column=1, sticky="ew", pady=6, padx=(4, 0))
        ...
        ttk.Label(frame, text=tips, justify="left").grid(row=6, column=0, columnspan=2, sticky="w")
